...
if [ -f $DB_NAME ]; then
  # force shutdown and don't permit new connections
  print "/usr/local/firebird/bin/gfix -user SYSDBA -password $ISC_PASSWORD -shut full -force 0 $DB_NAME || exit 1"
  /usr/local/firebird/bin/gfix -user SYSDBA -password $ISC_PASSWORD -shut full -force 0 $DB_NAME || exit 1
  echo "success"
else
